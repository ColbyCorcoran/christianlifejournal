...
struct ScripturePassageSelection: Equatable {
    ...
    func abbreviatedDisplayString(bibleBooks: [BibleBook]) -> String? {
        guard bookIndex >= 0 && bookIndex < bibleBooks.count else { return nil }
        let book = bibleBooks[bookIndex]
        // Use the second abbreviation (index 1) if available, otherwise fallback to name
        let abbreviation = book.abbreviations.count > 1 ? book.abbreviations[1] : book.name
        if verseEnd > verse {
            return "\(abbreviation) \(chapter):\(verse)-\(verseEnd)"
        } else {
